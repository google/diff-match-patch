...
export default class diff_match_patch {
  ...
  diff_xIndex(diffs: Diff[], loc: number): number {
    ...
    for (x = 0; x < diffs.length; x++) {
      if (diffs[x].operation !== DIFF_INSERT) {  // Equality or deletion.
        chars1 += diffs[x].text.length;
      }
      if (diffs[x].operation !== DIFF_DELETE) {  // Equality or insertion.
        ...
        if (text1 == text2) {
          ...
          if (text1.length > this.Match_MaxBits &&
            ...
            for (var y = 0; y < patches[x].diffs.length; y++) {
              ...
              if (mod.operation === DIFF_INSERT) {  // Insertion
                text = text.substring(0, start_loc + index2) + mod.text +
                  text.substring(start_loc + index2);
              } else if (mod.operation === DIFF_DELETE) {  // Deletion
