...
int diff_match_patch_test::run_all_tests() {
  ...
  runTest(
      std::bind(&diff_match_patch_test::testDiffCleanupSemanticLossless, this));
...
TEST_F(diff_match_patch_test, testDiffCommonPrefix) {
  ...
  assertEquals("diff_commonPrefix: nullptr case.", 0,
               dmp.diff_commonPrefix("abc", "xyz"));

  assertEquals("diff_commonPrefix: Non-nullptr case.", 4,
...
TEST_F(diff_match_patch_test, testDiffCommonSuffix) {
  ...
  assertEquals("diff_commonSuffix: nullptr case.", 0,
               dmp.diff_commonSuffix("abc", "xyz"));

  assertEquals("diff_commonSuffix: Non-nullptr case.", 4,
...
TEST_F(diff_match_patch_test, testDiffCommonOverlap) {
  ...
  assertEquals("diff_commonOverlap: nullptr case.", 0,
  ...
  assertEquals("diff_commonOverlap: Unicode.", 0,
               dmp.diff_commonOverlap(L"fi", std::wstring(L"\ufb01i")));
}
...
TEST_F(diff_match_patch_test, testDiffHalfmatch) {
  ...
  assertEquals("diff_halfMatch: Non-optimal halfmatch.",
               TStringVector({L"qHillo", L"w", L"x", L"Hulloy", L"HelloHe"}),
               dmp.diff_halfMatch("qHilloHelloHew", "xHelloHeHulloy"));

  dmp.Diff_Timeout = 0;
  assertEmpty("diff_halfMatch: Optimal no halfmatch.",
              dmp.diff_halfMatch(L"qHilloHelloHew", L"xHelloHeHulloy"));
}
...
TEST_F(diff_match_patch_test, testDiffLinesToChars) {
  ...
  tmpVarList.emplace_back(
      NUtils::to_wstring({1, 2, 1}));  //(("\u0001\u0002\u0001"));
  tmpVarList.emplace_back(
      NUtils::to_wstring({2, 1, 2}));  // (("\u0002\u0001\u0002"));
...
TEST_F(diff_match_patch_test, testDiffCharsToLines) {
  ...
  TDiffVector diffs;
  diffs.emplace_back(EQUAL,
                     NUtils::to_wstring({1, 2, 1}));  // ("\u0001\u0002\u0001");
  diffs.emplace_back(INSERT,
                     NUtils::to_wstring({2, 1, 2}));  // ("\u0002\u0001\u0002");
  ...
  assertEquals("diff_charsToLines: More than 256.",
               TDiffVector({Diff(DELETE, lines)}), diffs);
}
...
TEST_F(diff_match_patch_test, testDiffCleanupMerge) {
  ...
  assertEquals("diff_cleanupMerge: Slide edit left.",
               TDiffVector({Diff(INSERT, "ab"), Diff(EQUAL, "ac")}), diffs);

  diffs = {Diff(EQUAL, "c"), Diff(INSERT, "ab"), Diff(EQUAL, "a")};
  dmp.diff_cleanupMerge(diffs);
  assertEquals("diff_cleanupMerge: Slide edit right.",
               TDiffVector({Diff(EQUAL, "ca"), Diff(INSERT, "ba")}), diffs);

  diffs = {Diff(EQUAL, "a"), Diff(DELETE, "b"), Diff(EQUAL, "c"),
           Diff(DELETE, "ac"), Diff(EQUAL, "x")};
  dmp.diff_cleanupMerge(diffs);
  assertEquals("diff_cleanupMerge: Slide edit left recursive.",
               TDiffVector({Diff(DELETE, "abc"), Diff(EQUAL, "acx")}), diffs);

  diffs = {Diff(EQUAL, "x"), Diff(DELETE, "ca"), Diff(EQUAL, "c"),
           Diff(DELETE, "b"), Diff(EQUAL, "a")};
  dmp.diff_cleanupMerge(diffs);
  assertEquals("diff_cleanupMerge: Slide edit right recursive.",
               TDiffVector({Diff(EQUAL, "xca"), Diff(DELETE, "cba")}), diffs);
}
...
TEST_F(diff_match_patch_test, testDiffCleanupSemanticLossless) {
  ...
  diffs = {Diff(EQUAL, "The c"), Diff(INSERT, "ow and the c"),
           Diff(EQUAL, "at.")};
  dmp.diff_cleanupSemanticLossless(diffs);
  assertEquals("diff_cleanupSemantic: Word boundaries.",
               TDiffVector({Diff(EQUAL, "The "), Diff(INSERT, "cow and the "),
                            Diff(EQUAL, "cat.")}),
               diffs);
  ...
  assertEquals("diff_cleanupSemantic: Hitting the start.",
               TDiffVector({Diff(DELETE, "a"), Diff(EQUAL, "aax")}), diffs);

  diffs = {Diff(EQUAL, "xa"), Diff(DELETE, "a"), Diff(EQUAL, "a")};
  dmp.diff_cleanupSemanticLossless(diffs);
  assertEquals("diff_cleanupSemantic: Hitting the end.",
               TDiffVector({Diff(EQUAL, "xaa"), Diff(DELETE, "a")}), diffs);

  diffs = {Diff(EQUAL, "The xxx. The "), Diff(INSERT, "zzz. The "),
           Diff(EQUAL, "yyy.")};
  dmp.diff_cleanupSemanticLossless(diffs);
  assertEquals("diff_cleanupSemantic: Sentence boundaries.",
               TDiffVector({Diff(EQUAL, "The xxx."), Diff(INSERT, " The zzz."),
                            Diff(EQUAL, " The yyy.")}),
               diffs);
}
...
TEST_F(diff_match_patch_test, testDiffCleanupSemantic) {
  ...
  diffs = {Diff(DELETE, "ab"), Diff(INSERT, "cd"), Diff(EQUAL, "12"),
           Diff(DELETE, "e")};
  dmp.diff_cleanupSemantic(diffs);
  assertEquals("diff_cleanupSemantic: No elimination #1.",
               TDiffVector({Diff(DELETE, "ab"), Diff(INSERT, "cd"),
                            Diff(EQUAL, "12"), Diff(DELETE, "e")}),
               diffs);

  diffs = {Diff(DELETE, "abc"), Diff(INSERT, "ABC"), Diff(EQUAL, "1234"),
           Diff(DELETE, "wxyz")};
  dmp.diff_cleanupSemantic(diffs);
  assertEquals("diff_cleanupSemantic: No elimination #2.",
               TDiffVector({Diff(DELETE, "abc"), Diff(INSERT, "ABC"),
  ...
  assertEquals("diff_cleanupSemantic: Simple elimination.",
               TDiffVector({Diff(DELETE, "abc"), Diff(INSERT, "b")}), diffs);

  diffs = {Diff(DELETE, "ab"), Diff(EQUAL, "cd"), Diff(DELETE, "e"),
           Diff(EQUAL, "f"), Diff(INSERT, "g")};
  dmp.diff_cleanupSemantic(diffs);
  assertEquals("diff_cleanupSemantic: Backpass elimination.",
               TDiffVector({Diff(DELETE, "abcdef"), Diff(INSERT, "cdfg")}),
               diffs);

  diffs = {Diff(INSERT, "1"), Diff(EQUAL, "A"),  Diff(DELETE, "B"),
  ...
  assertEquals("diff_cleanupSemantic: Multiple elimination.",
               TDiffVector({Diff(DELETE, "AB_AB"), Diff(INSERT, "1A2_1A2")}),
               diffs);

  diffs = {Diff(EQUAL, "The c"), Diff(DELETE, "ow and the c"),
           Diff(EQUAL, "at.")};
  dmp.diff_cleanupSemantic(diffs);
  assertEquals("diff_cleanupSemantic: Word boundaries.",
               TDiffVector({Diff(EQUAL, "The "), Diff(DELETE, "cow and the "),
                            Diff(EQUAL, "cat.")}),
               diffs);
  ...
  assertEquals("diff_cleanupSemantic: Reverse overlap elimination.",
               TDiffVector({Diff(INSERT, "def"), Diff(EQUAL, "xxx"),
                            Diff(DELETE, "abc")}),
               diffs);

  diffs = {Diff(DELETE, "abcd1212"), Diff(INSERT, "1212efghi"),
           Diff(EQUAL, "----"), Diff(DELETE, "A3"), Diff(INSERT, "3BC")};
  dmp.diff_cleanupSemantic(diffs);
  assertEquals(
      "diff_cleanupSemantic: Two overlap eliminations.",
      TDiffVector({Diff(DELETE, "abcd"), Diff(EQUAL, "1212"),
                   Diff(INSERT, "efghi"), Diff(EQUAL, "----"),
                   Diff(DELETE, "A"), Diff(EQUAL, "3"), Diff(INSERT, "BC")}),
      diffs);
}
...
TEST_F(diff_match_patch_test, testDiffCleanupEfficiency) {
  ...
  diffs = {Diff(DELETE, "ab"), Diff(INSERT, "12"), Diff(EQUAL, "wxyz"),
           Diff(DELETE, "cd"), Diff(INSERT, "34")};
  dmp.diff_cleanupEfficiency(diffs);
  assertEquals(
      "diff_cleanupEfficiency: No elimination.",
      TDiffVector({Diff(DELETE, "ab"), Diff(INSERT, "12"), Diff(EQUAL, "wxyz"),
                   Diff(DELETE, "cd"), Diff(INSERT, "34")}),
      diffs);

  diffs = {Diff(DELETE, "ab"), Diff(INSERT, "12"), Diff(EQUAL, "xyz"),
           Diff(DELETE, "cd"), Diff(INSERT, "34")};
  dmp.diff_cleanupEfficiency(diffs);
  assertEquals("diff_cleanupEfficiency: Four-edit elimination.",
               TDiffVector({Diff(DELETE, "abxyzcd"), Diff(INSERT, "12xyz34")}),
               diffs);

  diffs = {Diff(INSERT, "12"), Diff(EQUAL, "x"), Diff(DELETE, "cd"),
           Diff(INSERT, "34")};
  dmp.diff_cleanupEfficiency(diffs);
  assertEquals("diff_cleanupEfficiency: Three-edit elimination.",
               TDiffVector({Diff(DELETE, "xcd"), Diff(INSERT, "12x34")}),
               diffs);

  diffs = {Diff(DELETE, "ab"), Diff(INSERT, "12"), Diff(EQUAL, "xy"),
           Diff(INSERT, "34"), Diff(EQUAL, "z"),   Diff(DELETE, "cd"),
           Diff(INSERT, "56")};
  dmp.diff_cleanupEfficiency(diffs);
  assertEquals(
      "diff_cleanupEfficiency: Backpass elimination.",
      TDiffVector({Diff(DELETE, "abxyzcd"), Diff(INSERT, "12xy34z56")}), diffs);
  ...
  assertEquals(
      ...
      TDiffVector({Diff(DELETE, "abwxyzcd"), Diff(INSERT, "12wxyz34")}), diffs);
  dmp.Diff_EditCost = 4;
}
...
TEST_F(diff_match_patch_test, testDiffDelta) {
  ...
  auto diffs = TDiffVector({Diff(EQUAL, "jump"), Diff(DELETE, "s"),
                            Diff(INSERT, "ed"), Diff(EQUAL, " over "),
                            Diff(DELETE, "the"), Diff(INSERT, "a"),
  ...
  assertEquals("diff_toDelta:", L"=4\t-1\t+ed\t=6\t-3\t+a\t=5\t+old dog",
               delta);
  ...
  assertThrow("diff_fromDelta: Too long.",
              dmp.diff_fromDelta(text1 + L"x", delta), std::wstring);

  // Generates error (19 > 18).
  assertThrow("diff_fromDelta: Too short.",
              dmp.diff_fromDelta(text1.substr(1), delta), std::wstring);

  // Generates error (%c3%xy invalid Unicode).
  assertThrow("diff_fromDelta: Invalid character.",
              dmp.diff_fromDelta("", "+%c3%xy"), std::wstring);

  // Test deltas with special characters.
  diffs = {
      Diff(EQUAL, std::wstring(L"\u0680 ") + kZero + std::wstring(L" \t %")),
      Diff(DELETE, std::wstring(L"\u0681 ") + kOne + std::wstring(L" \n ^")),
      Diff(INSERT, std::wstring(L"\u0682 ") + kTwo + std::wstring(L" \\ |"))};

  text1 = dmp.diff_text1(diffs);
  golden = std::wstring(L"\u0680 ") + kZero + std::wstring(L" \t %\u0681 ") +
           kOne + std::wstring(L" \n ^");
...
TEST_F(diff_match_patch_test, testDiffXIndex) {
  ...
  auto diffs = TDiffVector(
      {Diff(DELETE, "a"), Diff(INSERT, "1234"), Diff(EQUAL, "xyz")});
...
TEST_F(diff_match_patch_test, testDiffLevenshtein) {
  auto diffs = TDiffVector(
      {Diff(DELETE, "abc"), Diff(INSERT, "1234"), Diff(EQUAL, "xyz")});
...
TEST_F(diff_match_patch_test, testDiffBisect) {
  ...
  auto diffs =
      TDiffVector({Diff(DELETE, "c"), Diff(INSERT, "m"), Diff(EQUAL, "a"),
...
TEST_F(diff_match_patch_test, testDiffMain) {
  ...
  assertEquals("diff_main: RHS side nullptr case.", diffs,
               dmp.diff_main("abc", "", false));

  diffs = {Diff(INSERT, "abc")};
  assertEquals("diff_main: LHS side nullptr case.", diffs,
               dmp.diff_main("", "abc", false));
  ...
  diffs = {Diff(DELETE, "Apple"), Diff(INSERT, "Banana"),
           Diff(EQUAL, "s are a"), Diff(INSERT, "lso"), Diff(EQUAL, " fruit.")};
  assertEquals(
      "diff_main: Simple case #2.", diffs,
      dmp.diff_main("Apples are a fruit.", "Bananas are also fruit.", false));

  diffs = {Diff(DELETE, "a"), Diff(INSERT, L"\u0680"), Diff(EQUAL, "x"),
           Diff(DELETE, "\t"), Diff(INSERT, NUtils::to_wstring(kZero))};
  assertEquals("diff_main: Simple case #3.", diffs,
               dmp.diff_main(L"ax\t", std::wstring(L"\u0680x") + kZero, false));

  diffs = {Diff(DELETE, "1"), Diff(EQUAL, "a"),  Diff(DELETE, "y"),
  ...
  diffs = {Diff(INSERT, " "), Diff(EQUAL, "a"), Diff(INSERT, "nd"),
...
TEST_F(diff_match_patch_test, testMatchMain) {
  ...
  assertEquals("match_main: nullptr text.", -1,
               dmp.match_main("", "abcdef", 1));

  assertEquals("match_main: nullptr pattern.", 3,
...
TEST_F(diff_match_patch_test, testPatchObj) {
  ...
  p.length1 = 18;
  p.length2 = 17;
  p.diffs = {Diff(EQUAL, "jump"),   Diff(DELETE, "s"),   Diff(INSERT, "ed"),
             Diff(EQUAL, " over "), Diff(DELETE, "the"), Diff(INSERT, "a"),
             Diff(EQUAL, "\nlaz")};
  std::wstring strp =
      L"@@ -21,18 +22,17 @@\n jump\n-s\n+ed\n  over \n-the\n+a\n %0Alaz\n";
...
TEST_F(diff_match_patch_test, testPatchApply) {
  ...
  std::wstring resultStr =
      results.first + L"\t" + std::to_wstring(boolArray.size());
  ...
  assertEquals("patch_apply: Exact match.",
               L"That quick brown fox jumped over a lazy dog.\ttrue\ttrue",
               resultStr);
  ...
  assertEquals("patch_apply: Partial match.",
               L"That quick red rabbit jumped over a tired tiger.\ttrue\ttrue",
               resultStr);
  ...
  assertEquals("patch_apply: Failed match.",
               L"I am the very model of a modern major general.\tfalse\tfalse",
               resultStr);
  ...
  boolArray = results.second;
  resultStr = results.first + NUtils::to_wstring(boolArray);
  assertEquals("patch_apply: Big delete, small change.", L"xabcy\ttrue\ttrue",
               resultStr);
  ...
  boolArray = results.second;
  resultStr = results.first + NUtils::to_wstring(boolArray);
  assertEquals("patch_apply: Big delete, large change 2.", L"xabcy\ttrue\ttrue",
               resultStr);
  dmp.Patch_DeleteThreshold = 0.5f;
  ...
  boolArray = results.second;
  resultStr = results.first + L"\t" + NUtils::to_wstring(boolArray[0], false);
  assertEquals("patch_apply: Near edge exact match.", L"XtestY\ttrue",
               resultStr);
...
TEST_F(diff_match_patch_test, fromGitHubExamples) {
  auto lhs =
      L"I am the very model of a modern Major-General, I've information "
      ...
      L"categorical.";
  auto rhs =
      L"I am the very model of a cartoon individual, My animation's comical, "
