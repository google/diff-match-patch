...
std::wstring Diff::toString() const
{
    ...
    return std::wstring( L"Diff(" ) + strOperation( operation ) + std::wstring( L",\"" ) + prettyText + std::wstring( L"\")" );
}
...
TDiffVector diff_match_patch::diff_compute( const std::wstring &text1, const std::wstring &text2, bool checklines, clock_t deadline )
{
    ...
    if ( !hm.empty() )
    {
        ...
        const TDiffVector diffs_b = diff_main( text1_b, text2_b, checklines, deadline );
        // Merge the results.
        ...
        if ( diffs[ pointer - 1 ].operation == DELETE && diffs[ pointer ].operation == INSERT )
        {
            ...
            if ( overlap_length1 >= overlap_length2 )
            {
                if ( overlap_length1 >= deletion.length() / 2.0 || overlap_length1 >= insertion.length() / 2.0 )
                {
                    ...
                    diffs.emplace( diffs.begin() + pointer, EQUAL, insertion.substr( 0, overlap_length1 ) );
                    diffs[ pointer - 1 ].text = deletion.substr( 0, deletion.length() - overlap_length1 );
            ...
            else
            {
                if ( overlap_length2 >= deletion.length() / 2.0 || overlap_length2 >= insertion.length() / 2.0 )
                {
                    ...
                    diffs.emplace( diffs.begin() + pointer, EQUAL, deletion.substr( 0, overlap_length2 ) );
                    diffs[ pointer - 1 ].operation = INSERT;
                    diffs[ pointer - 1 ].text = insertion.substr( 0, insertion.length() - overlap_length2 );
            ...
            case EQUAL:
                ...
                if ( count_delete + count_insert > 1 )
                {
                    if ( count_delete != 0 && count_insert != 0 )
                    {
                        ...
                        if ( commonlength != 0 )
                        {
                            if ( ( pointer > ( count_delete + count_insert ) ) && diffs[ pointer - ( count_delete + count_insert ) - 1 ].operation == EQUAL )
                            {
                                diffs[ pointer - count_delete - count_insert - 1 ].text += text_insert.substr( 0, commonlength );
                            }
                            else
                            {
                                diffs.emplace( diffs.begin(), EQUAL, text_insert.substr( 0, commonlength ) );
                                pointer++;
                            }
                        ...
                        if ( commonlength != 0 )
                        {
                            diffs[ pointer ].text = safeMid( text_insert, text_insert.length() - commonlength ) + diffs[ pointer ].text;
...
std::wstring diff_match_patch::diff_prettyHtml( const TDiffVector &diffs )
{
    ...
    for ( auto &&aDiff : diffs )
    {
        ...
        {
            case INSERT:
                html += std::wstring( L"<ins style=\"background:#e6ffe6;\">" ) + text + std::wstring( L"</ins>" );
                break;
            case DELETE:
                html += std::wstring( L"<del style=\"background:#ffe6e6;\">" ) + text + std::wstring( L"</del>" );
                break;
            case EQUAL:
...
{
    // Check for null inputs not needed since null can't be passed via std::wstring
            ...
            if ( ( rd[ j ] & matchmask ) != 0 )
            {
                ...
                if ( score <= score_threshold )
                {
                    ...
                    if ( best_loc > loc )
                    {
                        ...
                        start = std::max( 1ULL, ( 2 * loc > best_loc ) ? 2 * loc - best_loc : 1 );
                    }
                    else
                    {
...
void diff_match_patch::patch_addContext( Patch &patch, const std::wstring &text )
{
    ...
    while ( ( text.find( pattern ) != text.rfind( pattern ) ) && ( pattern.length() < ( Match_MaxBits - Patch_Margin - Patch_Margin ) ) )
    {
        ...
        pattern = safeMid( text, std::max( 0ULL, ( ( patch.start2 > padding ) ? patch.start2 - padding : 0ULL ) ), std::min( text.length(), patch.start2 + patch.length1 + padding ) - std::max( 0ULL, ( patch.start2 > padding ) ? patch.start2 - padding : 0 ) );
    }
    // Add one chunk for good luck.
    padding += Patch_Margin;

    // Add the prefix.
    std::wstring prefix = safeMid( text, std::max( 0ULL, ( ( patch.start2 > padding ) ? patch.start2 - padding : 0ULL ) ), patch.start2 - std::max( 0ULL, ( ( patch.start2 > padding ) ? patch.start2 - padding : 0ULL ) ) );
    if ( !prefix.empty() )
    {
...
std::wstring diff_match_patch::patch_addPadding( TPatchVector &patches )
{
    ...
    if ( patches.front().diffs.empty() || patches.front().diffs.front().operation != EQUAL )
    {
        // Add nullPadding equality.
        patches.front().diffs.emplace( patches.front().diffs.begin(), EQUAL, nullPadding );
    ...
    //patch = patches.back();
    //diffs = patch.diffs;
    if ( ( patches.back().diffs.size() == 0 ) || patches.back().diffs.back().operation != EQUAL )
    {
        // Add nullPadding equality.
...
void diff_match_patch::patch_splitMax( TPatchVector &patches )
{
    ...
    for ( int x = 0; x < patches.size(); x++ )
    {
        ...
        while ( !bigpatch.diffs.empty() )
        {
            ...
            precontext = precontext.substr( std::max( 0ULL, ( precontext.length() > Patch_Margin ) ? ( precontext.length() - Patch_Margin ) : 0 ) );
            ...
            if ( postcontext.length() != 0 )
            {
                ...
                if ( ( patch.diffs.size() != 0 ) && ( patch.diffs[ patch.diffs.size() - 1 ].operation == EQUAL ) )
                {
