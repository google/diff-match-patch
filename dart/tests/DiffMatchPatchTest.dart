...
class Expect {
  static void equals(var expected, var actual, String msg) {
    ...
    throw new Exception('Expect.equals(expected: <$expected>, actual: <$actual> $msg) fails.');
  }
...
void testDiffHalfmatch() {
  ...
  Expect.listEquals(['12', '90', 'a', 'z', '345678'], dmp.test_diff_halfMatch('1234567890', 'a345678z')!,
      'diff_halfMatch: Single Match #1.');

  Expect.listEquals(['a', 'z', '12', '90', '345678'], dmp.test_diff_halfMatch('a345678z', '1234567890')!,
      'diff_halfMatch: Single Match #2.');

  Expect.listEquals(['abc', 'z', '1234', '0', '56789'], dmp.test_diff_halfMatch('abc56789z', '1234567890')!,
      'diff_halfMatch: Single Match #3.');

  Expect.listEquals(['a', 'xyz', '1', '7890', '23456'], dmp.test_diff_halfMatch('a23456xyz', '1234567890')!,
      'diff_halfMatch: Single Match #4.');

  Expect.listEquals(['12123', '123121', 'a', 'z', '1234123451234'],
      dmp.test_diff_halfMatch('121231234123451234123121', 'a1234123451234z')!, 'diff_halfMatch: Multiple Matches #1.');

  Expect.listEquals(
      ['', '-=-=-=-=-=', 'x', '', 'x-=-=-=-=-=-=-='],
      dmp.test_diff_halfMatch('x-=-=-=-=-=-=-=-=-=-=-=-=', 'xx-=-=-=-=-=-=-=')!,
      'diff_halfMatch: Multiple Matches #2.');

  Expect.listEquals(
      ['-=-=-=-=-=', '', '', 'y', '-=-=-=-=-=-=-=y'],
      dmp.test_diff_halfMatch('-=-=-=-=-=-=-=-=-=-=-=-=y', '-=-=-=-=-=-=-=yy')!,
      'diff_halfMatch: Multiple Matches #3.');

  // Optimal diff would be -q+x=H-i+e=lloHe+Hu=llo-Hew+y not -qHillo+x=HelloHe-w+Hulloy
  Expect.listEquals(['qHillo', 'w', 'x', 'Hulloy', 'HelloHe'],
      dmp.test_diff_halfMatch('qHilloHelloHew', 'xHelloHeHulloy')!, 'diff_halfMatch: Non-optimal halfmatch.');
...
void testDiffLinesToChars() {
  ...
  assertLinesToCharsResultEquals({
    ...
    'chars2': '\u0002',
    'lineArray': ['', 'a', 'b']
  }, dmp.test_diff_linesToChars('a', 'b'), 'diff_linesToChars: No linebreaks.');
  ...
  lineList.insert(0, '');
  assertLinesToCharsResultEquals({'chars1': chars, 'chars2': '', 'lineArray': lineList},
      dmp.test_diff_linesToChars(lines, ''), 'diff_linesToChars: More than 256.');
}
...
void testDiffCleanupMerge() {
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([new Diff(Operation.equal, 'a'), new Diff(Operation.delete, 'b'), new Diff(Operation.insert, 'c')],
      diffs, 'diff_cleanupMerge: No change case.');
  ...
  ], diffs, 'diff_cleanupMerge: Prefix and suffix detection.');
  ...
  ], diffs, 'diff_cleanupMerge: Prefix and suffix detection with equalities.');

  diffs = [new Diff(Operation.equal, 'a'), new Diff(Operation.insert, 'ba'), new Diff(Operation.equal, 'c')];
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([new Diff(Operation.insert, 'ab'), new Diff(Operation.equal, 'ac')], diffs,
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([new Diff(Operation.equal, 'ca'), new Diff(Operation.insert, 'ba')], diffs,
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals([new Diff(Operation.equal, 'xca'), new Diff(Operation.delete, 'cba')], diffs,
  ...
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals(
      [new Diff(Operation.insert, 'a'), new Diff(Operation.equal, 'bc')], diffs, 'diff_cleanupMerge: Empty merge.');

  diffs = [new Diff(Operation.equal, ''), new Diff(Operation.insert, 'a'), new Diff(Operation.equal, 'b')];
  dmp.diff_cleanupMerge(diffs);
  Expect.listEquals(
      [new Diff(Operation.insert, 'a'), new Diff(Operation.equal, 'b')], diffs, 'diff_cleanupMerge: Empty equality.');
}
...
void testDiffCleanupSemanticLossless() {
  ...
  ], diffs, 'diff_cleanupSemanticLossless: Blank lines.');
  ...
  ], diffs, 'diff_cleanupSemanticLossless: Line boundaries.');
  ...
  ], diffs, 'diff_cleanupSemanticLossless: Word boundaries.');
  ...
  Expect.listEquals([
    ...
    new Diff(Operation.insert, 'cow-and-the-'),
    new Diff(Operation.equal, 'cat.')
  ], diffs, 'diff_cleanupSemanticLossless: Alphanumeric boundaries.');

  diffs = [new Diff(Operation.equal, 'a'), new Diff(Operation.delete, 'a'), new Diff(Operation.equal, 'ax')];
  dmp.test_diff_cleanupSemanticLossless(diffs);
  Expect.listEquals([new Diff(Operation.delete, 'a'), new Diff(Operation.equal, 'aax')], diffs,
  ...
  dmp.test_diff_cleanupSemanticLossless(diffs);
  Expect.listEquals([new Diff(Operation.equal, 'xaa'), new Diff(Operation.delete, 'a')], diffs,
  ...
  ], diffs, 'diff_cleanupSemanticLossless: Sentence boundaries.');
}
...
void testDiffCleanupSemantic() {
  ...
  Expect.listEquals([
    ...
    new Diff(Operation.equal, '12'),
    new Diff(Operation.delete, 'e')
  ], diffs, 'diff_cleanupSemantic: No elimination #1.');
  ...
  Expect.listEquals([
    ...
    new Diff(Operation.equal, '1234'),
    new Diff(Operation.delete, 'wxyz')
  ], diffs, 'diff_cleanupSemantic: No elimination #2.');

  diffs = [new Diff(Operation.delete, 'a'), new Diff(Operation.equal, 'b'), new Diff(Operation.delete, 'c')];
  dmp.diff_cleanupSemantic(diffs);
  Expect.listEquals([new Diff(Operation.delete, 'abc'), new Diff(Operation.insert, 'b')], diffs,
  ...
  dmp.diff_cleanupSemantic(diffs);
  Expect.listEquals([new Diff(Operation.delete, 'abcdef'), new Diff(Operation.insert, 'cdfg')], diffs,
  ...
  dmp.diff_cleanupSemantic(diffs);
  Expect.listEquals([new Diff(Operation.delete, 'AB_AB'), new Diff(Operation.insert, '1A2_1A2')], diffs,
  ...
  Expect.listEquals([
    ...
    new Diff(Operation.equal, 'cat.')
  ], diffs, 'diff_cleanupSemantic: Word boundaries.');

  diffs = [new Diff(Operation.delete, 'abcxx'), new Diff(Operation.insert, 'xxdef')];
  dmp.diff_cleanupSemantic(diffs);
  Expect.listEquals([new Diff(Operation.delete, 'abcxx'), new Diff(Operation.insert, 'xxdef')], diffs,
      'diff_cleanupSemantic: No overlap elimination.');

  diffs = [new Diff(Operation.delete, 'abcxxx'), new Diff(Operation.insert, 'xxxdef')];
  dmp.diff_cleanupSemantic(diffs);
  Expect.listEquals(
      [new Diff(Operation.delete, 'abc'), new Diff(Operation.equal, 'xxx'), new Diff(Operation.insert, 'def')],
      diffs,
  ...
  dmp.diff_cleanupSemantic(diffs);
  Expect.listEquals(
      [new Diff(Operation.insert, 'def'), new Diff(Operation.equal, 'xxx'), new Diff(Operation.delete, 'abc')],
      diffs,
  ...
  ], diffs, 'diff_cleanupSemantic: Two overlap eliminations.');
}
...
void testDiffCleanupEfficiency() {
  ...
  Expect.listEquals([
    ...
    new Diff(Operation.delete, 'cd'),
    new Diff(Operation.insert, '34')
  ], diffs, 'diff_cleanupEfficiency: No elimination.');
  ...
  dmp.diff_cleanupEfficiency(diffs);
  Expect.listEquals([new Diff(Operation.delete, 'abxyzcd'), new Diff(Operation.insert, '12xyz34')], diffs,
  ...
  dmp.diff_cleanupEfficiency(diffs);
  Expect.listEquals([new Diff(Operation.delete, 'xcd'), new Diff(Operation.insert, '12x34')], diffs,
  ...
  dmp.diff_cleanupEfficiency(diffs);
  Expect.listEquals([new Diff(Operation.delete, 'abxyzcd'), new Diff(Operation.insert, '12xy34z56')], diffs,
  ...
  dmp.diff_cleanupEfficiency(diffs);
  Expect.listEquals([new Diff(Operation.delete, 'abwxyzcd'), new Diff(Operation.insert, '12wxyz34')], diffs,
...
void testDiffDelta() {
  ...
  diffs = [
    new Diff(Operation.equal, '\u0680 \x00 \t %'),
    new Diff(Operation.delete, '\u0681 \x01 \n ^'),
  ...
  delta = dmp.diff_toDelta(diffs);
  Expect.equals(
      '+A-Z a-z 0-9 - _ . ! ~ * \' ( ) ; / ? : @ & = + \$ , # ', delta, 'diff_toDelta: Unchanged characters.');
...
void testDiffMain() {
  ...
  Expect.listEquals(
      diffs, dmp.diff_main('Apples are a fruit.', 'Bananas are also fruit.', false), 'diff_main: Simple case #2.');
  ...
  Expect.listEquals(diffs, dmp.diff_main('ABCDa=bcd=efghijklmnopqrsEFGHIJKLMNOefg', 'a-bcd-efghijklmnopqrs', false),
      'diff_main: Overlap #3.');
  ...
  Expect.listEquals(diffs, dmp.diff_main('a [[Pennsylvania]] and [[New', ' and [[Pennsylvania]]', false),
...
void testMatchBitap() {
  ...
  Expect.equals(
      -1, dmp.test_match_bitap('abcdefghijklmnopqrstuvwxyz', 'abcdefg', 24), 'match_bitap: Distance test #1.');

  Expect.equals(
      0, dmp.test_match_bitap('abcdefghijklmnopqrstuvwxyz', 'abcdxxefg', 1), 'match_bitap: Distance test #2.');
...
void testMatchMain() {
  ...
  Expect.equals(4, dmp.match_main('I am the very model of a modern major general.', ' that berry ', 5),
...
void testPatchApply() {
  ...
  resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
  Expect.equals(
      'That quick red rabbit jumped over a tired tiger.\ttrue\ttrue', resultStr, 'patch_apply: Partial match.');

  results = dmp.patch_apply(patches, 'I am the very model of a modern major general.');
  boolArray = results[1];
  resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
  Expect.equals(
      'I am the very model of a modern major general.\tfalse\tfalse', resultStr, 'patch_apply: Failed match.');
  ...
  boolArray = results[1];
  resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
  Expect.equals('xabc12345678901234567890---------------++++++++++---------------12345678901234567890y\tfalse\ttrue',
  ...
  boolArray = results[1];
  resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
  Expect.equals('ABCDEFGHIJKLMNOPQRSTUVWXYZ--------------------1234567YYYYYYYYYY890\tfalse\ttrue', resultStr,
